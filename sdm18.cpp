...
//使うピン番の設定
BufferedSerial sensor(PA_9,PA_10,921600);//なんか知らんけど921600が一番いい
//BufferedSerial pc(USBTX,USBRX,115200);//シリアルをとりたいときに開放する
...
void sdm_all(){
    ...
    while(true){
        ...
        if(prev == 0xA5 && data == 0x20){
            ...
            uint16_t crc =calculate_crc16(buf, 18);//crcの計算結果を出してるけど実際は全然使ってない
            uint16_t distance = static_cast<uint16_t>(buf[6]) | (static_cast<uint16_t>(buf[7]) << 8);
            if(distance != 0xFFFF){
                distance +=OFFSET;//補正
                if(distance > MAX_LIMIT){//極大値がでてきたらそのデータは上位マイコンに送らない
                    continue;
                }
